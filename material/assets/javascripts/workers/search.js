      ...
      extract(value, (block, type, start, end) => {
        if (type & 1) {
          ...
          split(section, lunr.tokenizer.separator, ([index2, until]) => {
            ...
            table[block].push(start + index2 << 12 | until - index2 << 2 | type);
            tokens.push(new lunr.Token(section.slice(index2, until).toLowerCase(), {
              position: block << 20 | table[block].length - 1
            }));
          });
        } else {
          table[block] || (table[block] = []);
          table[block].push(start << 12 | end - start << 2 | type);
        }
      });
    }
    return tokens;
          ...
          const groups = this.index.search(query).reduce((item, { ref, score, matchData }) => {
            ...
            if (typeof document2 !== "undefined") {
              ...
              for (const [field, positions] of Object.entries(map))
                document2[field] = highlighter(document2[field], this.tables[document2.location][field], positions);
